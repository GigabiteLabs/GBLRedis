...
const fs = require('fs')
const log = require('./client/utilities/logger')
const Setup = require('./client/setup/setup-config')
...
class GBLRedis {
    constructor(){
        this.log = log('GBLRedis')
        this.env = require('./client/constants/env')
        this.constants = require('./client/constants/app-constants')
        this.msgs = require('./client/constants/messages')
    ...
    async init() {
        ...
        const setup = await new Setup(this.eventEmitter)
        this.redis = await setup.getClient(this.eventEmitter)
    }
    ...
    async get(key,table){
        ...
        return new Promise(function(resolve, reject){
            ...
            try {
                let concatenatedKey = (table) ? `${table}:${key}` : key
                self.redis.hgetall(concatenatedKey, function (err, obj) {
                    if(err) throw err
                    if(obj){
                        resolve(obj)
                    }else{
    ...
    async set(key, value, table, userExp){
        ...
        return new Promise(function(resolve, reject){
            ...
            try {
                ...
                self.handleExp(concatenatedKey, userExp)
            } catch (e) {
                self.log.error(`Error: ${e}`)
                reject(e)
            }
    ...
    async update(key, value, table){
        ...
        return new Promise(async function(resolve, reject){
            ...
            try {
                ...
                let concatenatedKey = (table) ? `${table}:${key}` : key
                self.redis.hmset(concatenatedKey, newObj, function (err, res) {
                    if(err) throw err
                    if(res == 'OK'){
                        resolve(res)
                    }else{
